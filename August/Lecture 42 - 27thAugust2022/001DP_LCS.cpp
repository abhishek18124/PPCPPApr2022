...
int LCS(string s1, string s2, int m, int n, int i, int j) {
	...
	if(s1[i] == s2[j]) {
		// compute the length of the LCS b/w s1[i+1...m-1] and s2[j+1...n-1]
		return 1 + LCS(s1, s2, m, n, i+1, j+1);
	}
	
	// compute the length of the LCS b/w s1[i+1...m-1] and s2[j...n-1]
	int X = LCS(s1, s2, m, n, i+1, j);

	// compute the length of the LCS b/w s1[i...m-1] and s2[j+1...n-1]
	int Y = LCS(s1, s2, m, n, i, j+1);
